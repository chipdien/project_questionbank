...
import { X, Save, Loader2, Image, Sparkles } from 'lucide-react';
  ...
  return (
    <AnimatePresence mode="wait">
      {isOpen && localQuestion && (
        <div key="question-edit-modal-main" className="fixed inset-0 z-100 flex items-center justify-center p-4">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          <motion.div
            ...
            className={`relative bg-surface rounded-2xl shadow-xl w-full ${statement.includes('```bbt') ? 'max-w-7xl' : 'max-w-4xl'} max-h-[90vh] flex flex-col z-10 border border-outline-variant/20 overflow-hidden`}
          >
            {/* Header */}
      ...
      {ocrTarget !== null && (
        <div key="question-edit-modal-ocr" className="fixed inset-0 z-200 flex items-center justify-center p-4">
