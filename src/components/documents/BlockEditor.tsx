...
export default function BlockEditor({ block, onChange, onRemove, qNumber }: BlockEditorProps) {
  ...
  const renderContent = () => {
    ...
    if (block.type === 'question') {
      ...
      const displayNum = q.manualNumber !== undefined ? q.manualNumber : qNumber;
      const statement = cleanMathpixData(q.statement || q.content || '');
              ...
              <div className="flex gap-4 items-center border-b border-outline-variant/20 pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    value={q.manualNumber !== undefined ? q.manualNumber : (displayNum || '')}
                    onChange={(e) => setLocalQuestion((prev: any) => ({ ...prev, manualNumber: e.target.value }))}
                  />
                </div>
                <div className="text-[10px] text-outline italic">Để trống để dùng số tự động</div>
              </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {q.options.map((opt: any, idx: number) => (
                      <div key={idx} className="flex gap-3 items-start bg-surface-container-high p-2 rounded-lg border border-outline-variant/10">
                        ...
                        <textarea
                          ...
                          value={(opt.content || opt.statement || '').replace(/\\\\/g, '\\')}
                          onChange={(e) => {
